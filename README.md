...
- 附近停車場：路外停車場即時剩餘車位（需 TDX）
- 附近 YouBike：站點即時可借／可還，一般車與電輔車分開顯示（需 TDX）
- 收藏：捷運站／公車路線／站牌，儲存於 localStorage
- 設定：可勾選要顯示哪些功能，沒勾的功能其按鈕與分頁都會隱藏（存 localStorage，預設 YouBike 關、其餘全開）
- 深色模式、手機定位（附近站牌為模擬資料）
...
- 附近站牌／附近停車場／附近 YouBike：座標查詢無法預先同步，維持即時呼叫 TDX（每個功能各自有一份 60 秒節流，互不影響）。YouBike 站點基本資料走 `/v2/Bike/Station`、即時可借可還走 `/v2/Bike/Availability`，以 `StationUID` 對起來
